...
int main(){
    ...
    Capsule simcapsule(m_capsule_init, m_heatshield, m_fuel, CD_capsule, A_capsule, chute, x_ballistic_init);
    ...
    ballistic_descent_info_file << bd_time << ","
                            << simcapsule.x_ballistic(0) << ","
                            << simcapsule.x_ballistic(1) << ","
                            << simcapsule.x_ballistic(2) << ","
                            << total_dtheta
                            << "\n";
    
    while (simcapsule.x_ballistic(2) >= h_chute_deploy) { // propagate ballistic dynamics and count dtheta for orbit optimization
        bd_time = bd_time + EDL_DT;
        
        total_dtheta = total_dtheta + (simcapsule.x_ballistic(0)*cos(simcapsule.x_ballistic(1))*EDL_DT)/(R_EARTH + simcapsule.x_ballistic(2));
        x_ballistic_next = propagateBallistic(simcapsule);
        simcapsule.x_ballistic = x_ballistic_next;
    ...
    for ( int i = 1; i <= NUM_ORBITS_BEFORE_INCL_CHANGE*numIterationsPerOrbit; i++) {
        ...
        sat.u_LVLH(0) = -1*(K(0,0)*sat.x_LVLH(0) + K(0,1)*sat.x_LVLH(1) + K(0,2)*sat.x_LVLH(2) + K(0,3)*sat.x_LVLH(3) + K(0,4)*sat.x_LVLH(4) + K(0,5)*sat.x_LVLH(5));
        sat.u_LVLH(1) = -1*(K(1,0)*sat.x_LVLH(0) + K(1,1)*sat.x_LVLH(1) + K(1,2)*sat.x_LVLH(2) + K(1,3)*sat.x_LVLH(3) + K(1,4)*sat.x_LVLH(4) + K(1,5)*sat.x_LVLH(5));
        sat.u_LVLH(2) = -1*(K(2,0)*sat.x_LVLH(0) + K(2,1)*sat.x_LVLH(1) + K(2,2)*sat.x_LVLH(2) + K(2,3)*sat.x_LVLH(3) + K(2,4)*sat.x_LVLH(4) + K(2,5)*sat.x_LVLH(5));
    ...
    cout << "Desired Inclination (rads): " << iDesired;
    ...
    while (!orbitParams.hasDeployed)
    {
        ...
        if (abs(orbitParams.desiredDropLocation(0) - sat.x_LLA(0)) <= orbitParams.latTolerance)
        {
            if (!orbitParams.hasOrbitedOnce)
            {
                ...
                cout << "Satellite has passed over desired latitude once.";
            }
            else if (!orbitParams.hasOrbitedTwice && ((globalTime - orbitParams.timeOfFirstPass) >= 0.5 * T_ORBIT))
            {
                ...
                cout << "Satellite has passed over desired latitude twice.";
            }
        }
        ...
        if (orbitParams.hasOrbitedOnce && orbitParams.hasOrbitedTwice && !orbitParams.phaseShiftDetermined)
        {
            ...
            cout << "Longitudinal phase shift per orbit determined.";
        }
        ...
        if (orbitParams.phaseShiftDetermined && abs(orbitParams.desiredDropLocation(0) - sat.x_LLA(0)))
        {
            if (abs(sat.x_LLA(1) + orbitParams.lonPhaseShiftPerOrbit) <= orbitParams.lonTolerance)
            {
                ...
                cout << "Satellite ready to drop after timeToDropFromFinalPass elapses.";
            }
        }
    ...
    V_0_ballistic = sat.x_ECI(Eigen::seq(3, 5)).norm();                  // initial velocity, m/s
    gamma_0_ballistic = 5 * (PI / 180);                                  // initial flight path angle, rads
    h_0_ballistic = sat.x_ECI(Eigen::seq(0, 2)).norm() - R_EARTH;        // initial altitude, m
    x_ballistic_init << V_0_ballistic, gamma_0_ballistic, h_0_ballistic; // initial ballistic descent state
    ...
    { // propagate ballistic dynamics and count dtheta for orbit optimization
